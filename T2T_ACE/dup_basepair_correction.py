...
def extend_2_right(dup_interval, calling_reference_fasta, called_ref_aligner,
    ...
    while min(q_end_list) <= interval_size(new_interval):
        ...
        if q_end_count[-1][1] == 2:
            ...
            print(q_end_count)
            print(f"extend interval by {q_end_count[0][0] - interval_size(dup_interval)}")
...
def extend_2_left(dup_interval, calling_reference_fasta, called_ref_aligner,
    ...
    while min(q_start_list) <= 1:
        ...
        if q_start_count[0][1] == 2:
            print("new interval", new_interval, interval_size(new_interval))
            print(q_start_count)
            if window_size == q_start_count[-1][0]:
                print("The original pos is correct")
            else:
                print(f"extend interval by {q_start_count[-1][0]}")
            copies_alignment_start = new_pos + q_start_count[-1][0]
            break
        else:
            print("new interval", new_interval, interval_size(new_interval))
            print(q_start_count)
            max_q_start_percentage = (end - max(q_start_list) + 1) / interval_size(new_interval)
    ...
    return extended_2_left_interval